...
const Button = ({ theme, text, className, ...otherProps }) => {
  if (theme === "home"){
      return <HomePage className={className}>{text}</HomePage>;
        ...
        return <AuthPage className={className} {...otherProps}>{text}</AuthPage>;
    } else if(theme === 'rentFrom') {
        return <RentFrom className={className} {...otherProps}>{text}</RentFrom>;
    } else if(theme === 'filter') {
        return <Filter className={className} {...otherProps}>{text}</Filter>;
    } else if(theme === 'sort') {
        return <Sort className={className} {...otherProps}>{text}</Sort>;
    } else if(theme === 'productCart') {
        return <ProductCart className={className} {...otherProps}>{text}</ProductCart>;
    } else if(theme === 'uploadForm') {
        return <UploadForm className={className} {...otherProps}>{text}</UploadForm>;
    } else if(theme === 'uploadFormSubmit') {
        return <UploadFormSubmit className={className} {...otherProps}>{text}</UploadFormSubmit>;
    } else if(theme === 'logout') {
        return <Logout className={className} {...otherProps}>{text}</Logout>;
    } else if(theme === 'navUploadProduct') {
        return <NavUploadProduct className={className} {...otherProps}>{text}</NavUploadProduct>;
    } else if(theme === 'sentMessage') {
        return <SentMessage className={className} {...otherProps}>{text}</SentMessage>;
    } else if(theme === 'update') {
        return <UpdateImg className={className} {...otherProps}>{text}</UpdateImg>;
    } else if(theme === 'updateProduct') {
        return <UpdateProduct className={className} {...otherProps}>{text}</UpdateProduct>;
    } else if(theme === 'updateMsg') {
        return <UpdateMsg className={className} {...otherProps}>{text}</UpdateMsg>;
    } else if(theme === 'deleteProduct') {
        return <DeleteProduct className={className} {...otherProps}>{text}</DeleteProduct>;
    }
