...
exports.adminLogout = async(req, res) => {
 ...
 exports.isAdmin = async (req, res ,next) => {
    const isAdin = req.session.isAdmin;
    if(!isAdin) {
        return new Error('you not admin')
    } else if(isAdin){
        next()
    }
};
...
exports.getLastRentstUser = async (req, res) => {
    const limit = 10;
    try { 
        const lastRents = await Message.find({fromUser: req.params.id}).populate('productId').limit(limit)
        // const lastRents = await Message.find({fromUser: req.params.id}).limit(limit)
        //  lastRents = await Message.populate('productId')
         console.log(lastRents)
        res.status(200).json({
            status: "seccess",
            lastRents
        })
...
exports.getLastRentstProduct = async (req, res) => {
    const limit = 10;
    try { 
        const lastRents = await Message.find({productId: req.params.id}).populate('fromUser').limit(limit)
...
exports.getRentStats = async (req, res, next) => {
    ...
    try {
        ...
        // const todayRentStats = await Message.aggregate([
        //     {
        //         $project: {
        //             yearMonthDayUTC: {
        //                 formatDate: {
        ...
        // const todayRejectRentStats = await Message.aggregate([
        //     {
        //         $match:{ status: 'reject',
        //                  createdAt: {
        //                     $eq: new Date(currentDate),
        ...
        //     },
        //     {
        //         $count: 'rejectRentStats'
        //     }
        // ]);

        // const todayApproveRentStats = await Message.aggregate([
        //     {
        ...
        //     },
        //     {
        //         $count: 'approveRentStats'
        //     }
        // ]);

        // const totalApproveRentStats = await Message.aggregate([
        //     {
        //         $match:{ status: 'approve'}
        //     },
        //     {
        //         $count: 'totalApproveRentStats'
        //     }
        // ]);
...
exports.getLastProducts = async (req, res) => {
    try {
        const lastProducts = await Product.find().sort({createdAt: -1}).limit(5).populate('user');
